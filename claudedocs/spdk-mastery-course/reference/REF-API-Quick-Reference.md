# SPDK API Quick Reference

**Version**: SPDK main branch (2024)
**Last Updated**: 2026-03-30

This reference covers the most commonly used APIs across the major SPDK subsystems.
Each entry includes the function signature, a brief description, return value, and
typical usage context. For full documentation see the Doxygen output or the
...
2. [Thread and Poller](#2-thread-and-poller)
3. [I/O Channel](#3-io-channel)
4. [Block Device (Bdev)](#4-block-device-bdev)
5. [NVMe Driver](#5-nvme-driver)
6. [NVMe-oF Target](#6-nvme-of-target)
7. [Memory and Environment](#7-memory-and-environment)
8. [JSON-RPC](#8-json-rpc)
9. [Key Types and Callbacks](#9-key-types-and-callbacks)
...
The application framework initializes the DPDK EAL, sets up SPDK reactors, starts
the JSON-RPC server, and drives the main event loop. Most applications call only
`spdk_app_opts_init`, populate `spdk_app_opts`, then call `spdk_app_start`.
...
**Description**: Fills `*opts` with safe default values. Always call this before
setting individual fields, because new fields are added at the end of the struct
and the `opts_size` argument guards against ABI mismatches.

**Parameters**:
- `opts` - Pointer to the options structure to initialize.
...
**Description**: Initializes SPDK, starts the reactor loop, then calls `start_fn`
...
**Description**: Requests graceful application shutdown. The reactor loop will
...
**Description**: Initiates the application shutdown sequence. Similar to
...
**Description**: Releases all resources allocated by `spdk_app_start`. Must be
...
**Description**: Creates a new SPDK thread. The thread is not associated with
...
**Description**: Returns the SPDK thread currently running on the calling OS
thread. Returns NULL if called from a non-SPDK context.

**Returns**: Current `spdk_thread *`, or NULL.
...
**Description**: Returns the application (main) thread created by
`spdk_app_start`. This is the thread on which initialization callbacks run.

**Returns**: App thread pointer, or NULL if called before `spdk_app_start`.
...
**Description**: Marks a thread for exit. The thread will stop polling and be
...
**Returns**: 0 on success, -EBUSY if the thread still has active resources.
...
**Description**: Frees memory associated with a thread that has already exited
...
**Description**: Schedules `fn(ctx)` to run on the specified thread. This is
...
**Returns**: 0 on success, -ENOMEM if the message ring is full.
...
**Description**: Registers a polling function on the current SPDK thread.
...
**Returns**: Opaque poller handle, or NULL on failure.
...
**Description**: Same as `spdk_poller_register` but attaches a human-readable
...
**Description**: Stops and frees a poller. The pointer is set to NULL after
...
**Description**: Marks a poller as capable of operating in interrupt mode.
...
**Description**: Returns the driver-private context buffer associated with an
I/O channel. The size of this buffer is specified when registering the I/O device.

**Returns**: Pointer to the per-channel context.
...
**Description**: Releases a reference to an I/O channel. When the reference
...
**Description**: Opens a block device by name and returns a descriptor. The
...
**Description**: Closes a bdev descriptor. All I/O channels obtained from
...
**Description**: Returns a per-thread I/O channel for the given bdev descriptor.
...
**Returns**: I/O channel pointer, or NULL on failure.
...
**Description**: Submits an asynchronous read in byte units. `buf` must be
...
**Returns**: 0 if the I/O was queued, -ENOMEM if no I/O resources available,
...
**Description**: Same as `spdk_bdev_read` but parameters are in block units
...
**Description**: Submits an asynchronous write in byte units. `buf` must be
DMA-safe memory and must remain valid until the completion callback fires.

**Returns**: 0 if queued, -ENOMEM or other negative errno on error.
...
**Description**: Same as `spdk_bdev_write` but parameters are in block units.
...
**Description**: Submits a TRIM/UNMAP command in byte units. The device may
reclaim the storage. Not all bdev backends support unmap; check with
`spdk_bdev_io_type_supported`.

**Returns**: 0 on success, -ENOTSUP if unsupported, other negative errno on error.
...
**Description**: Writes zeros to the specified byte range. More efficient than
...
**Description**: Issues a flush (cache writeback) for the given byte range.
...
**Description**: Returns a completed bdev I/O back to the pool. Must be called
...
**Description**: Scans for NVMe devices matching `trid`. For each candidate
...
**Description**: Detaches from an NVMe controller and frees all associated
...
**Description**: Returns the number of namespaces reported by the controller.
...
**Description**: Returns the namespace handle for the given 1-based namespace
...
**Description**: Allocates an I/O submission/completion queue pair on the
...
**Returns**: Queue pair pointer, or NULL on failure.
...
**Description**: Destroys an I/O queue pair and frees its resources. Must be
...
**Description**: Submits a read command to the namespace. `payload` must be
...
**Returns**: 0 if submitted, negative errno on error.
...
**Description**: Submits a write command to the namespace. Same parameter
...
**Description**: Submits an NVMe Flush command to force pending data to
non-volatile media.
...
**Description**: Polls the completion queue and invokes callbacks for completed
commands. Call this repeatedly from an SPDK poller. Pass `max_completions = 0`
to process all available completions.

**Returns**: Number of completions processed (>= 0), or negative errno on error.
...
**Description**: Polls the admin completion queue. Must be called periodically
...
**Description**: Submits a raw admin command. Use for vendor-specific or
...
**Description**: Creates a new NVMe-oF target. The target is the top-level
...
**Returns**: Target pointer, or NULL on failure.
...
**Description**: Asynchronously destroys a target. The callback is invoked
...
**Description**: Creates a new NVMe-oF subsystem with the given NQN (NVMe
...
**Returns**: Subsystem pointer, or NULL on failure.
...
**Description**: Transitions a subsystem from the INACTIVE state to ACTIVE,
making it accessible to initiators. The callback fires once the transition
completes (or fails).

**Returns**: 0 if the transition was initiated, negative errno on error.
...
**Description**: Transitions a subsystem from ACTIVE to INACTIVE, disconnecting
...
**Description**: Adds a listen address to a subsystem. Initiators connect to
this address to reach the subsystem. `trid` specifies transport type, address,
and port (e.g., TCP on `192.168.1.1:4420`).

**Returns**: void (result delivered via `cb_fn`).
...
**Description**: Adds a bdev as a namespace to the subsystem. The NSID is
...
**Returns**: Assigned NSID (>= 1) on success, 0 on failure.
...
**Description**: Creates a transport of the given type (e.g., `"TCP"` or
`"RDMA"`) asynchronously. The transport must be added to a target via
`spdk_nvmf_tgt_add_transport` before use.

**Returns**: 0 if creation was initiated, negative errno on error.
...
**Description**: Attaches a transport to a target. After this completes
...
**Description**: Allocates DMA-capable or shared memory. `flags` controls the
...
**Description**: Same as `spdk_malloc` but the returned buffer is zeroed.
Preferred when the buffer will be used as a read destination or initialized
to zero.

**Returns**: Pointer to zeroed buffer, or NULL on failure.
...
**Description**: Resizes a buffer previously allocated with `spdk_malloc` or
`spdk_zmalloc`. Existing data is preserved. The buffer may be moved.

**Returns**: Pointer to resized buffer, or NULL on failure.
...
**Description**: Frees a buffer allocated with `spdk_malloc`, `spdk_zmalloc`,
...
**Description**: Alias for `spdk_free`. Frees DMA memory returned by legacy
...
**Description**: Initializes `spdk_env_opts` with default values. Use when
...
**Description**: Creates a named pool of fixed-size objects backed by hugepage
...
**Description**: Destroys a mempool and frees its memory. All elements must
...
**Description**: Gets one element from the pool. Returns immediately without
blocking. Returns NULL if the pool is empty.

**Returns**: Element pointer, or NULL if pool is empty.
...
**Description**: Returns one element to the pool.
...
**Description**: Batch variants for getting/returning multiple elements at once.
...
**Header**: `spdk/rpc.h`, `spdk/jsonrpc.h`
...
**Description**: Registers `func` as the handler for the JSON-RPC method named
...
**Description**: Begins writing the result field of a JSON-RPC response.
Returns a write context that accepts JSON values. Must be paired with
`spdk_jsonrpc_end_result`.

**Returns**: JSON write context pointer.
...
**Description**: Finalizes and sends the JSON-RPC response. After this call
...
**Description**: Convenience wrapper that sends a JSON-RPC result of `true`
...
**Description**: Sends a JSON-RPC error response with the given error code and
...
**Description**: Programmatic (non-macro) method registration. Prefer the
...
**Description**: Starts an RPC server listening on the given address (UNIX
...
**Description**: Accepts and processes pending RPC connections. Must be called
...
**Description**: Transitions the RPC server's state. Methods whose `state_mask`
