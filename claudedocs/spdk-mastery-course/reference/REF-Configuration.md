# SPDK Configuration Reference

**Course**: SPDK Mastery
**Document Type**: Reference
**Scope**: Application startup options, JSON configuration, RPC methods, environment variables, device binding

---

## Table of Contents

1. [Application Command-Line Options](#1-application-command-line-options)
2. [JSON Configuration File Format](#2-json-configuration-file-format)
3. [RPC Methods: Block Devices (bdev)](#3-rpc-methods-block-devices-bdev)
4. [RPC Methods: NVMe-oF Target](#4-rpc-methods-nvme-of-target)
5. [RPC Methods: iSCSI Target](#5-rpc-methods-iscsi-target)
6. [RPC Methods: Vhost](#6-rpc-methods-vhost)
7. [Environment Variables](#7-environment-variables)
8. [Hugepage Configuration](#8-hugepage-configuration)
9. [Device Binding: setup.sh](#9-device-binding-setupsh)
10. [RPC Socket and CLI Reference](#10-rpc-socket-and-cli-reference)

---

## 1. Application Command-Line Options

SPDK applications share a common set of options derived from `struct spdk_app_opts` (defined in `include/spdk/event.h`). These options control the DPDK environment, CPU assignment, memory, PCI access, and tracing.

### 1.1 Core Framework Options
...
The `--wait-for-rpc` flag (or `-w`) prevents subsystems from starting until an explicit RPC call is sent. This allows configuration to be loaded before any subsystem allocates resources.
...
## 2. JSON Configuration File Format

SPDK supports loading an initial configuration from a JSON file at startup (`-c` / `--config`). The file encodes a sequence of RPC method calls to be executed during initialization.

### 2.1 File Structure

The JSON configuration file is an array of RPC method objects. Each object has a `method` field naming the RPC, and a `params` object containing the arguments.
...
The `subsystems` array is processed in order. Subsystems that have dependencies must appear after the subsystems they depend on. Typical ordering:
...
### 2.3 Generating a JSON Config from a Running Instance
...
## 3. RPC Methods: Block Devices (bdev)

All bdev RPC calls can be sent to the running SPDK application via `rpc.py` or any JSON-RPC 2.0 client. The default socket path is `/var/tmp/spdk.sock`.

### 3.1 General bdev Options
...
An in-memory block device backed by hugepage memory. Used for testing and development.
...
A no-op block device that discards all writes and returns zeroes for reads. Useful for performance benchmarking with zero overhead.
...
### 3.11 Bdev Discovery and Status
...
## 4. RPC Methods: NVMe-oF Target

The NVMe-oF target exposes local bdevs as NVMe namespaces to remote initiators over TCP, RDMA, or FC transports.

### 4.1 Create a Transport
...
### 4.2 Create a Subsystem

A subsystem corresponds to one NVMe-oF target with a unique NQN.
...
### 4.6 NVMe-oF Status and Statistics
...
### 4.7 Complete NVMe-oF Setup Example
...
## 5. RPC Methods: iSCSI Target

The iSCSI target exposes bdevs as SCSI LUNs accessible via standard iSCSI initiators.

### 5.1 Global iSCSI Options
...
### 5.2 Create a Portal Group

A portal group defines the IP:port pairs that the target listens on.
...
### 5.3 Create an Initiator Group

An initiator group defines which initiator hosts are allowed to connect.
...
### 5.6 Complete iSCSI Setup Example
...
The Vhost target exposes bdevs as virtio-blk or virtio-scsi devices to QEMU virtual machines via Unix domain sockets.

### 6.1 Vhost Block Controller
...
### 6.5 QEMU Command to Use Vhost Controllers
...
### 7.3 DPDK Environment Variables
...
## 8. Hugepage Configuration

SPDK relies on Linux hugepages for all DMA-capable memory allocations. Proper hugepage configuration is critical for performance and stability.
...
SPDK's `setup.sh` uses the system default hugepage size. To use 1 GB pages, configure at boot time via kernel parameters.

### 8.2 Manual Hugepage Allocation
...
### 8.3 1 GB Hugepages (Boot Configuration)
...
### 8.4 Verifying Hugepage Allocation
...
### 8.6 Hugepage Memory Layout

SPDK calculates the required hugepages as follows:
...
The `--mem-size` / `-s` application flag requests memory from the already-allocated hugepage pool. It must be less than or equal to the total allocated hugepage memory.

---

## 9. Device Binding: setup.sh
...
## 10. RPC Socket and CLI Reference
