...
1. [애플리케이션 초기화 패턴](#1-application-initialization-pattern)
2. [Bdev 열기 / 읽기 / 쓰기 / 닫기 패턴](#2-bdev-open--read--write--close-pattern)
3. [완료 콜백을 사용한 비동기 I/O 패턴](#3-async-io-with-completion-callback-pattern)
4. [I/O 큐 대기 (ENOMEM) 패턴](#4-io-queue-wait-enomem-pattern)
5. [스레드 생성 및 메시지 전달 패턴](#5-thread-creation-and-message-passing-pattern)
6. [폴러 등록 패턴 (타이머 및 바쁜 폴링)](#6-poller-registration-pattern-timed-and-busy)
7. [I/O 채널 관리 패턴](#7-io-channel-management-pattern)
8. [RPC 메서드 등록 패턴](#8-rpc-method-registration-pattern)
9. [오류 처리 패턴](#9-error-handling-patterns)
10. [리소스 정리 패턴 (역순)](#10-resource-cleanup-pattern-reverse-order)
11. [NVMe 프로브 / 연결 패턴](#11-nvme-probe--attach-pattern)
12. [JSON-RPC를 통한 설정 패턴](#12-configuration-via-json-rpc-pattern)
...
### 사용 시점
전체 SPDK 프레임워크가 필요한 모든 SPDK 애플리케이션: EAL(DPDK), 리액터 시스템,
bdev 계층, RPC 서버, 서브시스템 초기화. 모든 SPDK 앱(타겟, 도구, 예제)의
필수 시작점입니다.
...
### 참고 사항
- `spdk_app_opts_init()`는 `opts` 필드를 설정하기 전에 **반드시** 호출해야 합니다 — ABI 안전을 위해
  and sets internal size for ABI safety.
- `spdk_app_start()`는 블로킹합니다. 모든 로직은 SPDK 스레드의 콜백 내부에서 실행됩니다.
- `spdk_app_fini()`는 `spdk_app_start()`가 반환된 **후에** 호출됩니다 — 리액터 내부에서
  the reactor.
- `spdk_app_start()`에 전달된 `arg1` 포인터는 `app_start()`의 첫 번째 인수가 됩니다.
- 소스 참조: `app/spdk_tgt/spdk_tgt.c`, `examples/bdev/hello_world/hello_bdev.c`

---

## 2. Bdev 열기 / 읽기 / 쓰기 / 닫기 패턴

### 사용 시점
애플리케이션이 bdev 추상화 계층을 통해 블록 디바이스(NVMe, malloc, AIO 등)에
I/O를 수행해야 할 때 사용합니다.
...
### 참고 사항
- `spdk_bdev_open_ext()`의 `bdev_event_cb` 파라미터는 **필수**입니다(NULL이면 안 됨).
- `spdk_bdev_desc_get_bdev()`는 원시 `spdk_bdev *`를 반환합니다 — 속성 쿼리를 위한 `spdk_bdev_get_*`에만
  property queries only; never cache it beyond the lifetime of the descriptor.
- `spdk_dma_zmalloc()`은 휴즈페이지 기반, 0으로 채워진, DMA 가능 메모리를 할당합니다.
  Always use it for I/O buffers; regular `malloc` will not work for NVMe/DMA.
- `spdk_bdev_get_buf_align()`은 bdev가 요구하는 최소 버퍼 정렬을 반환합니다.
- 소스 참조: `examples/bdev/hello_world/hello_bdev.c`

---

## 3. 완료 콜백을 사용한 비동기 I/O 패턴

### 사용 시점
SPDK의 모든 bdev I/O는 비동기입니다. bdev 계층을 통한 모든 읽기 또는 쓰기 작업에
이 패턴을 사용합니다. 호출자가 I/O를 제출하고 즉시 반환합니다. I/O가 완료되면
콜백이 발생합니다.
...
### 참고 사항
- `spdk_bdev_free_io()`는 모든 완료 콜백에서 정확히 한 번 **반드시** 호출해야 합니다.
- `success` 불리언이 기본 상태 표시자입니다. 버퍼를 사용하기 전에 확인하십시오.
- 오프셋과 길이는 블록이 아닌 **바이트** 단위입니다.
- 완료 콜백 내부에서 블로킹하지 마십시오 — 추가 작업을
  `spdk_thread_send_msg()` or chain the next I/O directly.
- 소스 참조: `examples/bdev/hello_world/hello_bdev.c`

---

## 4. I/O 큐 대기 (ENOMEM) 패턴

### 사용 시점
`spdk_bdev_read()` / `spdk_bdev_write()`가 `-ENOMEM`을 반환하면, bdev 계층이
내부 I/O 객체 풀을 소진한 것입니다. `spdk_bdev_queue_io_wait()`를 사용하여
...
## 5. 스레드 생성 및 메시지 전달 패턴

### 사용 시점
특정 SPDK 스레드에서 함수를 실행해야 할 때 — 새로 생성된 스레드이든
...
매 리액터 반복에서 실행해야 하는 작업(예: 완료 폴링)에는 **바쁜 폴러**(period = 0)를 사용합니다.
...
bdev에 I/O를 제출하는 모든 스레드는 자체 채널을 얻어야 합니다. 채널은
같은 스레드에서 획득하고 해제해야 합니다.
...
### 사용 시점
SPDK에 새 JSON-RPC 2.0 메서드를 추가할 때 — 구성, 쿼리 또는
...
### 사용 시점
NVMe 드라이버를 직접 사용할 때(bdev 계층 우회) — 최저 지연 시간 접근,
...
## 12. JSON-RPC를 통한 설정 패턴

### 사용 시점
실행 중인 SPDK 타겟의 런타임 설정 — bdev, 서브시스템, 리스너,
...
| 함정 | 올바른 접근 방법 |
|---|---|
| I/O 버퍼에 `malloc` 사용 | `spdk_bdev_get_buf_align`에서 적절한 정렬로 `spdk_dma_zmalloc` 사용 |
| 완료 콜백에서 `spdk_bdev_free_io()` 누락 | 모든 bdev 완료 콜백의 첫 줄로 항상 호출 |
| 다른 스레드의 채널 사용 | 같은 스레드에서 채널을 획득하고 사용; 스레드 경계를 넘으려면 메시지 전송 |
| 콜백 내에서 `spdk_app_fini()` 호출 | `spdk_app_start()`가 반환된 후 `main()`에서만 호출 |
| 초기화 순서로 리소스 해제 | 항상 역순: 폴러 -> 채널 -> 디스크립터 -> 스레드 -> 메모리 |
| bdev I/O의 `-ENOMEM` 무시 | 항상 `spdk_bdev_queue_io_wait` 재시도 패턴으로 처리 |
| `spdk_bdev_open_ext`에서 NULL `bdev_event_cb` | 항상 유효한 이벤트 콜백 함수를 제공 |
| `spdk_env_init` 없이 `spdk_nvme_probe` 호출 | `spdk_app_start`를 사용하지 않을 때 먼저 DPDK 환경 초기화 |
