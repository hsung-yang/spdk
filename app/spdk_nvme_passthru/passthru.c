...
static struct option g_cmdline_opts[] = {
	...
	{ "src-addr", required_argument, NULL, OPT_SRC_ADDR },
	{ "src-svcid", required_argument, NULL, OPT_SRC_SVCID },
...
usage(void)
{
	...
	printf("  --src-addr <addr>           Source address for fabrics connection (optional)\n");
	printf("  --src-svcid <port>          Source service id for fabrics connection (optional)\n");
...
parse_args(int ch, char *arg)
{
	...
	case OPT_TRSVCID:
		...
		break;
	case OPT_SRC_ADDR:
		snprintf(g_opts.src_addr, sizeof(g_opts.src_addr), "%s", arg);
		break;
	case OPT_SRC_SVCID:
		snprintf(g_opts.src_svcid, sizeof(g_opts.src_svcid), "%s", arg);
		break;
	case OPT_SUBNQN:
	 ...
	 struct spdk_nvme_ctrlr_opts *opts)
{
	...
	if (opts_cfg->src_addr[0] != '\0') {
		snprintf(opts->src_addr, sizeof(opts->src_addr), "%s", opts_cfg->src_addr);
	}
	if (opts_cfg->src_svcid[0] != '\0') {
		snprintf(opts->src_svcid, sizeof(opts->src_svcid), "%s", opts_cfg->src_svcid);
	}
...
passthru_run(void *arg1)
{
	...
	if (g_opts.src_addr[0] != '\0') {
		snprintf(ctrlr_opts.src_addr, sizeof(ctrlr_opts.src_addr), "%s", g_opts.src_addr);
	}
	if (g_opts.src_svcid[0] != '\0') {
		snprintf(ctrlr_opts.src_svcid, sizeof(ctrlr_opts.src_svcid), "%s", g_opts.src_svcid);
	}
	...
	while (!task.done) {
		...
		/*
		 * Keep polling admin completions even for I/O passthru so
		 * keep-alive commands can be serviced while long commands run.
		 */
		if (!g_opts.admin) {
			num_completions = spdk_nvme_ctrlr_process_admin_completions(g_ctrlr);
			if (num_completions < 0) {
				fprintf(stderr, "Failed processing admin completions: %s\n",
					spdk_strerror((int) - num_completions));
				rc = (int)num_completions;
				goto out;
			}
