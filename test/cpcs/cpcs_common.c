...
static bool g_accel_initialized;
...
static void
cpcs_accel_fini_cb(void *arg)
{
	(void)arg;
	g_accel_fini_done = true;
}
...
cpcs_test_init(struct cpcs_test_ctx *ctx)
{
	...
	if (!g_bdev_initialized) {
		...
		/*
		 * The accel framework must be up before any bdev (e.g. malloc)
		 * creates an I/O channel -- malloc_create_channel_cb pulls an
		 * accel channel for copy/crc offload. A standalone harness has no
		 * subsystem layer, so initialize it explicitly here.
		 */
		rc = spdk_accel_initialize();
		if (rc < 0) {
			SPDK_ERRLOG("Unable to initialize accel framework\n");
			return rc;
		}
		...
		if (g_env_refcount == 0) {
			if (g_bdev_initialized) {
				...
				if (g_accel_initialized) {
					g_accel_fini_done = false;
					spdk_accel_finish(cpcs_accel_fini_cb, NULL);
					while (!g_accel_fini_done) {
						cpcs_test_poll();
					}
		     ...
		     uint32_t nsid, uint64_t size_mb)
{
	...
	SPDK_NOTICELOG("Created test SLM bdev '%s': %llu MiB, NSID=%u\n",
		       name, (unsigned long long)size_mb, nsid);
	return 0;
}
