...
LEASE_MRS_BIN = DATA_DIR / "lease_mrs.bin"
LEASE_MEMFILL1_BIN = DATA_DIR / "lease_memfill_1.bin"
LEASE_MEMFILL2_BIN = DATA_DIR / "lease_memfill_2.bin"
...
PSLM_LEASE_CONFLICT_ATTEMPTS = int(env_default("PSLM_LEASE_CONFLICT_ATTEMPTS", "8"))
PSLM_LEASE_CONFLICT_SLEEP_SEC = float(env_default("PSLM_LEASE_CONFLICT_SLEEP_SEC", "0"))
PSLM_LEASE_CONFLICT_PARALLEL = int(env_default("PSLM_LEASE_CONFLICT_PARALLEL", "4"))
PSLM_LEASE_PRIMARY_LCORES = env_default("PSLM_LEASE_PRIMARY_LCORES", SPDK_NVME_PASSTHRU_LCORES)
PSLM_LEASE_SECONDARY_LCORES = env_default("PSLM_LEASE_SECONDARY_LCORES", PSLM_LEASE_PRIMARY_LCORES)
PSLM_LEASE_REQUIRE_CONFLICT = env_default("PSLM_LEASE_REQUIRE_CONFLICT", "0") == "1"

SLM_COPY_DESC_FMT_2H = 0x2
SLM_COPY_DESC_FMT_3H = 0x3
SLM_COPY_DESC_FMT_4H = 0x4
SLM_COPY_DESC_FMT_SHIFT = 8
DEMO_SCENARIO = env_default("DEMO_SCENARIO", "full")
...
def ensure_hugepages(target=1024):
    env_target = os.environ.get("CPCS_HUGEPAGES_TARGET")
    if env_target:
        try:
...
def start_target():
    ...
    run(["pkill", "-f", "spdk_tgt"], check=False)
    for _ in range(20):
    ...
    try:
        Path(RPC_SOCK).unlink(missing_ok=True)
    except Exception:
        pass

    for lock in Path("/var/tmp").glob("spdk_cpu_lock*"):
        try:
            lock.unlink()
        except Exception:
            pass
...
def main():
    ...
    parser.add_argument(
        "--scenario",
        choices=["full", "pslm-lease"],
        default=DEMO_SCENARIO,
        help="full runs the full PoC flow; pslm-lease runs only the pSLM lease conflict scenario.",
    )
    ...
    if not is_executable(SPDK_NVME_PASSTHRU):
        ...
        return 1
    if args.scenario == "full" and not (is_executable(NVME) or shutil.which(NVME)):
    ...
    if args.scenario == "pslm-lease":
        if slm_backend != "slm":
            print("pslm-lease scenario requires --slm-backend slm.", file=sys.stderr)
            return 1

        lease_range_len = (SLM_MB * 1024 * 1024) - 4096
        lease_range_len = (lease_range_len // 4096) * 4096
        if lease_range_len < 4096:
            print("SLM size too small for pSLM lease scenario (need at least 2 MiB).",
                  file=sys.stderr)
            return 1

        step(
            "Validate pSLM lease conflict via concurrent CPCS Execute",
            "Run overlapping built-in memfill executes in parallel and verify one fails while "
            "the other holds the lease.\n"
            "Expected: overlapping second execute is rejected until the first completes.",
        )

        with LEASE_MRS_BIN.open("wb") as f:
            f.write(struct.pack("<IIQ16s", SLM_NSID, lease_range_len, 0, b"\x00" * 16))

        lease_mrs_out = run_capture([
            SPDK_NVME_PASSTHRU,
            "--lcores", SPDK_NVME_PASSTHRU_LCORES,
            "--disable-cpumask-locks",
            "--no-rpc-server",
            "--admin-cmd", "--trtype", TRTYPE, "--traddr", TRADDR, "--trsvcid", TRSVCID,
            "--subnqn", NQN,
            "--opcode", "0x89", "--nsid", str(CPCS_NSID),
            "--cdw10", "0", "--cdw11", "1",
            "--data-len", "32", "--write", "--input-file", str(LEASE_MRS_BIN),
        ])
        print(color(lease_mrs_out, COLOR_LOG), end="")
        lease_rsid_hex = parse_result_hex(lease_mrs_out)
        if not lease_rsid_hex:
            print("Failed to parse RSID for pSLM lease scenario.", file=sys.stderr)
            return 1
        lease_rsid = int(lease_rsid_hex, 16)

        with LEASE_MEMFILL1_BIN.open("wb") as f:
            f.write(struct.pack("<QQQB7s", 1, 0, lease_range_len, 0xA5, b"\x00" * 7))
        with LEASE_MEMFILL2_BIN.open("wb") as f:
            f.write(struct.pack("<QQQB7s", 1, 0, lease_range_len, 0x5A, b"\x00" * 7))

        lease_conflict_seen = False
        attempts = max(1, PSLM_LEASE_CONFLICT_ATTEMPTS)
        parallel_reqs = max(1, PSLM_LEASE_CONFLICT_PARALLEL)
        for attempt in range(1, attempts + 1):
            first_cmd = [
                SPDK_NVME_PASSTHRU,
                "--lcores", PSLM_LEASE_PRIMARY_LCORES,
                "--disable-cpumask-locks",
                "--no-rpc-server",
                "--io-cmd", "--trtype", TRTYPE, "--traddr", TRADDR, "--trsvcid", TRSVCID,
                "--subnqn", NQN,
                "--opcode", "0x01", "--nsid", str(CPCS_NSID),
                "--cdw2", str((lease_rsid << 16) | 1), "--cdw3", "0", "--cdw4", "32",
                "--data-len", "32", "--write", "--input-file", str(LEASE_MEMFILL1_BIN),
            ]
            print(color(f"+ {quote_cmd(first_cmd)}", COLOR_CMD))
            first_proc = subprocess.Popen(first_cmd, stdout=subprocess.PIPE,
                                          stderr=subprocess.STDOUT, text=True)

            time.sleep(max(0.0, PSLM_LEASE_CONFLICT_SLEEP_SEC))

            second_cmd = [
                SPDK_NVME_PASSTHRU,
                "--lcores", PSLM_LEASE_SECONDARY_LCORES,
                "--disable-cpumask-locks",
                "--no-rpc-server",
                "--io-cmd", "--trtype", TRTYPE, "--traddr", TRADDR, "--trsvcid", TRSVCID,
                "--subnqn", NQN,
                "--opcode", "0x01", "--nsid", str(CPCS_NSID),
                "--cdw2", str((lease_rsid << 16) | 1), "--cdw3", "0", "--cdw4", "32",
                "--data-len", "32", "--write", "--input-file", str(LEASE_MEMFILL2_BIN),
            ]
            second_procs = []
            for _ in range(parallel_reqs):
                print(color(f"+ {quote_cmd(second_cmd)}", COLOR_CMD))
                second_procs.append(subprocess.Popen(second_cmd, stdout=subprocess.PIPE,
                                                     stderr=subprocess.STDOUT, text=True))

            first_out, _ = first_proc.communicate()
            if first_proc.returncode != 0:
                print(color(first_out or "", COLOR_LOG), end="")
                print("Primary execute failed unexpectedly during pSLM lease scenario.",
                      file=sys.stderr)
                return 1

            second_failed = False
            second_out = ""
            for proc in second_procs:
            ...
            if second_failed:
                lease_conflict_seen = True
                if second_out:
                    print(color(second_out, COLOR_LOG), end="")
                print(color(
                    f"pSLM lease conflict observed on attempt {attempt}/{attempts}.",
                    COLOR_RESULT))
                break

        if not lease_conflict_seen:
            warn_msg = (
                f"Failed to observe pSLM lease conflict after {attempts} attempts. "
                f"Increase SLM_MB/PSLM_LEASE_CONFLICT_ATTEMPTS or set "
                f"PSLM_LEASE_REQUIRE_CONFLICT=1 to fail hard."
            )
            if PSLM_LEASE_REQUIRE_CONFLICT:
                print(warn_msg, file=sys.stderr)
                return 1
            print(color(f"WARNING: {warn_msg}", COLOR_DESC))
        pause(args.auto)

        step(
            "Cleanup CPCS objects and stop target",
            "Remove CPCS/SLM objects and stop the target.",
        )
        cleanup_rpc(slm_backend)
        stop_target()
        pause(args.auto)
    ...
    step(
        ...
        f"  cdw12=0x{(SLM_COPY_DESC_FMT_2H << SLM_COPY_DESC_FMT_SHIFT):04x}       "
        f"desc_fmt={SLM_COPY_DESC_FMT_2H} (bits 11:8), nr=0 (one descriptor)\n"
    ...
    step(
        ...
        "  cdw11=0            Destination byte address (high)\n"
        f"  cdw12=0x{((SLM_COPY_DESC_FMT_2H << SLM_COPY_DESC_FMT_SHIFT) | (SLM_PAR_COPY_RANGE_COUNT - 1)):04x}       "
        f"desc_fmt={SLM_COPY_DESC_FMT_2H}, nr={SLM_PAR_COPY_RANGE_COUNT - 1}\n"
    ...
    step(
        ...
        "  cdw11=0            Destination byte address (high)\n"
        f"  cdw12=0x{(SLM_COPY_DESC_FMT_4H << SLM_COPY_DESC_FMT_SHIFT):04x}       "
        f"desc_fmt={SLM_COPY_DESC_FMT_4H} (bits 11:8), nr=0 (one descriptor)\n"
        "  data-len=32        one 32-byte copy descriptor\n"
    ...
    if slm_backend == "slm":
        ...
        for attempt in range(1, attempts + 1):
            ...
            print(color(f"+ {quote_cmd(second_cmd)}", COLOR_CMD))
            second_result = subprocess.run(second_cmd, text=True, capture_output=True)
            ...
            if second_result.returncode != 0:
                lease_conflict_seen = True
                second_out = (second_result.stdout or "") + (second_result.stderr or "")
                if "Cannot create lock on core" in second_out or "spdk_app_start failed" in second_out:
                    print("Lease scenario infrastructure error (lcore conflict). "
                          "Set non-overlapping PSLM_LEASE_*_LCORES values.", file=sys.stderr)
                    return 1
                if second_out:
                    print(color(second_out, COLOR_LOG), end="")
                print(color(
