...
function test_cpcs_namespace_creation() {
	...
	$rpc_py nvmf_create_subsystem $CPCS_TEST_SUBSYSTEM_NQN -s CPCSTEST001 -a -m 1024
	...
	$rpc_py nvmf_subsystem_add_ns $CPCS_TEST_SUBSYSTEM_NQN SLM0 -n $CPCS_MEMORY_NSID
...
function test_pslm_lease_scenario() {
	echo "=== Test: pSLM Lease Scenario ==="

	python3 "$testdir/pslm_lease_scenario.py" --spdk-root "$rootdir"
	if [ $? -eq 0 ]; then
		echo "PASS: pSLM lease scenario"
	else
		echo "FAIL: pSLM lease scenario"
		return 1
	fi
...
function run_cpcs_tests() {
	...
	test_pslm_lease_scenario || ((failures++))
