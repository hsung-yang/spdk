			    ...
			    uint32_t *range_count_out)
{
	...
	*sdaddr = ((uint64_t)cmd->cdw11 << 32) | cmd->cdw10;
	...
	for (i = 0; i < nr + 1; i++) {
		...
		if (src_ns == NULL || src_ns->bdev == NULL) {
			goto invalid_field;
		}
		...
		if (desc_fmt == SPDK_NVME_SLM_COPY_DESC_FMT_4H) {
			...
			struct spdk_nvmf_subsystem_pg_ns_info *src_info;
			uint32_t blocklen;
			...
			blocklen = spdk_bdev_get_block_size(src_ns->bdev);
			nbytes = ((uint64_t)ranges[i].nlb + 1) * blocklen;
			if (nbytes == 0 || (nbytes & 0x3) != 0) {
				goto invalid_field;
			}

			src_info = &req->qpair->group->sgroups[ctrlr->subsys->id].ns_info[snsid - 1];
			ranges[i].desc = src_ns->desc;
			ranges[i].ch = src_info->channel;
		}
