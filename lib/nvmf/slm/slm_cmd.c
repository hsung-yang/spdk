			    ...
			    uint32_t *range_count_out)
{
	...
	*range_count_out = 0;
	...
	desc_fmt = (enum spdk_nvme_slm_copy_desc_fmt)cmd->cdw12_bits.copy.df;
	...
	*sdaddr = ((uint64_t)cmd->cdw11 << 32) | cmd->cdw10;
	...
	if (desc_fmt == SPDK_NVME_SLM_COPY_DESC_FMT_4H) {
		copied = spdk_iov_xfer_to_buf(&ix, descs_4, desc_bytes);
	} else {
		copied = spdk_iov_xfer_to_buf(&ix, descs_23, desc_bytes);
	}

	if (copied != desc_bytes) {
		SPDK_ERRLOG("SLM COPY descriptor payload too short: copied=%zu expected=%u\n",
			    copied, desc_bytes);
		goto invalid_field;
	}
	...
	for (i = 0; i < nr + 1; i++) {
		...
		if (snsid == 0 || snsid == 0xFFFFFFFFu) {
			SPDK_ERRLOG("SLM COPY invalid source NSID at descriptor[%u]: snsid=%u\n",
				    i, snsid);
			goto invalid_field;
		}
		...
		if (src_ns == NULL) {
			if (_nvmf_subsystem_get_ns(ctrlr->subsys, snsid) != NULL) {
				SPDK_ERRLOG("SLM COPY source NSID %u exists but is not visible "
					    "to host %s (descriptor[%u])\n",
					    snsid, ctrlr->hostnqn, i);
			} else {
				SPDK_ERRLOG("SLM COPY source NSID %u not present in subsystem "
					    "(descriptor[%u])\n",
					    snsid, i);
			}
			goto invalid_field;
