...
_builtin_puid_for_pind(uint16_t pind)
{
	...
	case CPCS_BUILTIN_PIND_FILTER_AGG:
		return CPCS_BUILTIN_PUID_FILTER_AGG;
	case CPCS_BUILTIN_PIND_FILTERED_TOPK_EXACT:
		return CPCS_BUILTIN_PUID_FILTERED_TOPK_EXACT;
	case CPCS_BUILTIN_PIND_KV_PACK_STORE:
		return CPCS_BUILTIN_PUID_KV_PACK_STORE;
	case CPCS_BUILTIN_PIND_KV_UNPACK_LOAD:
		return CPCS_BUILTIN_PUID_KV_UNPACK_LOAD;
	case CPCS_BUILTIN_PIND_KV_LAYOUT_REPACK:
		return CPCS_BUILTIN_PUID_KV_LAYOUT_REPACK;
	case CPCS_BUILTIN_PIND_KV_BLOCK_SELECT:
		return CPCS_BUILTIN_PUID_KV_BLOCK_SELECT;
...
cpcs_program_index_is_builtin(uint16_t pind)
{
	return (pind == CPCS_BUILTIN_PIND_MEMCPY ||
		...
		pind == CPCS_BUILTIN_PIND_FILTER_AGG ||
		pind == CPCS_BUILTIN_PIND_FILTERED_TOPK_EXACT ||
		pind == CPCS_BUILTIN_PIND_KV_PACK_STORE ||
		pind == CPCS_BUILTIN_PIND_KV_UNPACK_LOAD ||
...
cpcs_program_install_builtins(struct spdk_nvmf_cpcs_ns *ns)
{
	...
	if (rc == 0) {
		rc = _install_one_builtin(ns, CPCS_BUILTIN_PIND_FILTER_AGG);
	}
	if (rc == 0) {
		rc = _install_one_builtin(ns, CPCS_BUILTIN_PIND_FILTERED_TOPK_EXACT);
	}
	if (rc == 0) {
		rc = _install_one_builtin(ns, CPCS_BUILTIN_PIND_KV_PACK_STORE);
	}
	if (rc == 0) {
		rc = _install_one_builtin(ns, CPCS_BUILTIN_PIND_KV_UNPACK_LOAD);
	}
	if (rc == 0) {
		rc = _install_one_builtin(ns, CPCS_BUILTIN_PIND_KV_LAYOUT_REPACK);
	}
	if (rc == 0) {
		rc = _install_one_builtin(ns, CPCS_BUILTIN_PIND_KV_BLOCK_SELECT);
	}
	if (rc == 0) {
		rc = _install_one_builtin(ns, CPCS_BUILTIN_PIND_KV_PREFIX_LOOKUP);
	}
	if (rc == 0) {
