...
cpcs_admin_load_program(struct spdk_nvmf_request *req)
{
	...
	pind = cmd->cdw10 & 0xFFFFu;
	ptype = (cmd->cdw10 >> 16) & 0xFFu;
	sel = (cmd->cdw10 >> 24) & 0x1u;
	pit = (cmd->cdw10 >> 25) & 0x7u;
	psize = cmd->cdw11;
	...
	if (sel != SPDK_NVME_CPCS_LOAD_OP_LOAD) {
...
cpcs_admin_mrs_management(struct spdk_nvmf_request *req)
{
	...
	/* Parse command fields */
	rsid = (cmd->cdw10 >> 16) & 0xFFFFu;
	sel = cmd->cdw10 & 0xFu;
	numr = cmd->cdw11 & 0xFFu;
